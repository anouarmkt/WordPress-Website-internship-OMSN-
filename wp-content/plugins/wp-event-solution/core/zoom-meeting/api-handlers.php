...
class Api_Handlers{
	 ...
	 *
	 */
	public function delete_meeting( $request_data, $type = 'meetings' ) {
		return \Etn\Core\Zoom_Meeting\Api::instance()->delete_meeting( $request_data, $type );
	}

	/**
	 * get all meeting function
	 *
	 */
	public function get_meetings( $user_id = 'me', $request_data = [], $type = 'meetings' ) {
		return \Etn\Core\Zoom_Meeting\Api::instance()->get_meetings( $user_id, $request_data, $type );
	}

	/**
	 * Get meeting details by id from API 
	 */
	public function get_meeting_details( $meeting_id, $type = 'meetings' ){
		return \Etn\Core\Zoom_Meeting\Api::instance()->get_meeting_details( $meeting_id, $type );
	}
	 ...
	 *
	 */
	public function get_user_settings( $user_id = 'me' ) {
		return \Etn\Core\Zoom_Meeting\Api::instance()->get_user_settings( $user_id );
	}
    ...
    public function meeting_info( $meeting_id ){
		...
		if ( !empty( $post_id ) ) {
			$id = $post_id[0]->ID ;
			$get_meeting_info['id'] = get_post_meta( $id ,'zoom_meeting_id',true );
	...
	public function meeting_type_list(){
		$type = [ 
			1 => 'Instant meeting', 2 => 'Scheduled meeting',
   ...
   /**
    ...
    public function create_meeting( $request_data ){
        ...
        if ( !empty( $request_data) ) {
			...
			$zoom_type = $request_data['type'];
			$hit_type_url = 'meetings';
			if ( $zoom_type == '2' ) {
				$zoom_settings = array_merge( $zoom_settings, $meeting_settings );
			} else if ( $zoom_type == '5' ) {
				$hit_type_url = 'webinars';
				$zoom_settings = array_merge( $zoom_settings, $webinar_settings );
			}

			$meeting_id = isset ( $request_data['meeting_id'] ) ? $request_data['meeting_id'] : 0;
			$user_id 	= isset ( $request_data['user_id'] ) ? $request_data['user_id'] : 0;

			$zoom_settings_data = [];
			if ( !empty ( $zoom_settings ) ) {
				$zoom_settings_data['auto_recording'] = $request_data[ 'auto_recording' ];
				
				// special case: question_and_answer for webinar
				if ( $zoom_type == '5' ) {
					$qa_enabled = 0;
					if ( isset( $request_data[ 'question_and_answer' ] ) && !empty( $request_data[ 'question_and_answer' ] ) ) {
						$qa_enabled = 1;
					}
					$zoom_settings_data['question_and_answer']['enable'] = $qa_enabled;
				}

				foreach ( $zoom_settings as $index => $setting_name ) {
					$zoom_settings_data[ $setting_name ] = !empty( $request_data[ $setting_name ] ) ? 1 : 0;
				}
			}
			...
			if ( !empty( $zoom_settings_data ) ) {
				$this->data['settings'] = $zoom_settings_data;
			}
			
            if ( empty( $meeting_id ) ) {
				$data = parent::init( 'users/' . $user_id . '/' . $hit_type_url , $this->data , 'POST' );
            } else {
				$data = parent::init( $hit_type_url . '/' . $meeting_id , $this->data , 'PATCH' );
			}
    ...
    public function delete_meeting( $request_data, $type = 'meetings' ) {
        if ( !empty( $request_data ) ) {
			...
			$this->data = [];
			$data = parent::init( $type . '/' . $meeting_id , $this->data , 'DELETE' );
	...
	/**
     * get all meeting or webinar of that user
     */
    public function get_meetings( $user_id = 'me', $request_data = [], $type = 'meetings' ) {
		$this->data = $request_data;
		...
		$data = parent::init( 'users/' . $user_id . '/' . $type, $this->data , 'GET' );
	 ...
	 */
	public function get_meeting_details( $meeting_id, $type = 'meetings' ) {
		return parent::init( $type . '/' . $meeting_id , [] ,'GET' );
	}

	/**
	 * Get user details settings 
	 */
	public function get_user_settings( $user_id = 'me' ) {
		return parent::init( 'users/' . $user_id . '/settings' , [] ,'GET' );
	}
}
