<?php
/**
 * The Conference Customizer Notification Section Class.
 *
 * @package The Conference
 */

/**
 * The_Conference_Customizer_Notice_Section class
 */
class The_Conference_Customizer_Notice_Section extends WP_Customize_Section {
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'the_conference_cust_notice_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'the_conference_cust_notice_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		$json = parent::json();
		global $the_conference_recommended_plugins;
		...
		$show_recommended_plugins = get_option( 'the_conference_show_recommended_plugins' );

		foreach ( $the_conference_recommended_plugins as $slug => $plugin_opt ) {
			...
			$the_conference_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$the_conference_recommended_plugin['class'] = 'active';
			} else {
				$the_conference_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$the_conference_recommended_plugin['button_class'] = 'install-now button';
					$the_conference_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$the_conference_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$the_conference_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$the_conference_recommended_plugin['button_class'] = 'deactivate-now button';
					$the_conference_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
            
			$info   = $this->call_plugin_api( $slug );
			$the_conference_recommended_plugin['id'] = $slug;
			$the_conference_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$the_conference_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$the_conference_recommended_plugin['description'] = $info->short_description;
			}

			$the_conference_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $the_conference_recommended_plugin;
				...
				<div class="recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="the-conference-recommeded-actions-container the-conference-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="the-conference-recommeded-actions">
